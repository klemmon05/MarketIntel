  ...
  return (
    <Page title={trigger.title}>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <form action={updateTriggerStatus} className="card space-y-2"><input type="hidden" name="id" value={trigger.id} /><select className="input" name="status">{Object.values(TriggerStatus).map(s => <option key={s} selected={s===trigger.status}>{s}</option>)}</select><textarea className="input" name="resolutionNote" placeholder="Required when resolving" /><button className="btn">Update Status</button></form>
